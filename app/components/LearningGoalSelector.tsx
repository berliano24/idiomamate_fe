...
const LEVELS = [
  { value: 'new', label: 'New', description: 'Just starting out' },
  { value: 'beginner', label: 'Beginner', description: 'Know some basics' },
  { value: 'expert', label: 'Expert', description: 'Fluent or advanced' },
];
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 p-6 md:p-8 mb-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] animate-fadeInUp">
      ...
      <div className="relative w-full h-[400px] mt-8 flex items-center justify-center overflow-hidden rounded-2xl bg-[#FCFAFB]">
        ...
        <button 
          onClick={handlePrev}
          className="absolute left-4 z-20 w-12 h-12 rounded-full bg-white/90 backdrop-blur shadow-md flex items-center justify-center text-gray-800 hover:scale-105 transition-transform"
        >
          <ChevronLeft size={24} />
        </button>
        <button 
          onClick={handleNext}
          className="absolute right-4 z-20 w-12 h-12 rounded-full bg-white/90 backdrop-blur shadow-md flex items-center justify-center text-gray-800 hover:scale-105 transition-transform"
        >
        ...
        <div className="relative w-full h-full flex items-center justify-center perspective-1000">
          {LANGUAGES.map((lang, index) => {
            ...
            // Circular logic for infinite feel (simplified)
            ...
            return (
              <div
                ...
                className={`absolute w-[240px] md:w-[280px] h-[340px] rounded-[2rem] p-6 cursor-pointer transition-all duration-500 ease-out flex flex-col shadow-xl
                  bg-gradient-to-br ${lang.gradient} text-white
                  ${isActive ? 'z-10 scale-100' : 'z-0 opacity-60'}`}
                style={{
                  transform: `translateX(${visualOffset * 180}px) scale(${isActive ? 1 : 0.85}) ${!isActive ? 'rotateY(' + (visualOffset * -15) + 'deg)' : ''}`,
                }}
              >
                {/* Watermark Code */}
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden rounded-[2rem]">
                  <span className="text-[140px] font-black opacity-10 select-none tracking-tighter">
                    {lang.code}
                  </span>
                </div>
                    ...
                    <div className="absolute inset-0 bg-black/40 backdrop-blur-md rounded-[2rem] p-6 flex flex-col justify-center animate-fadeIn z-20">
                      ...
                      <div className="mt-auto flex gap-2">
                        ...
                        <button 
                          ...
                          className="flex-1 py-3 rounded-full bg-[#E56A45] disabled:opacity-50 text-white font-bold text-sm"
                        >
                          Confirm
                        </button>
                      </div>
        ...
        <div className="absolute bottom-4 flex gap-2">
          {LANGUAGES.map((_, i) => (
            <div 
              key={i} 
              className={`h-1.5 rounded-full transition-all duration-300 ${activeIndex === i ? 'w-6 bg-[#E56A45]' : 'w-1.5 bg-gray-300'}`}
            />
          ))}
        </div>
      </div>
